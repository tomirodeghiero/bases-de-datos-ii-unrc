...
1. Crear tablas `banco` y `cotizacion`.
2. Implementar una funcion que:
   - reciba un banco;
   - calcule la cotizacion maxima de ese banco sin usar `MAX`;
   - actualice `banco.cotizacion_maxima`;
   - retorne ese valor en un parametro de salida.

## Script SQL completo

```sql
-- Esquema de trabajo del ejercicio
CREATE SCHEMA IF NOT EXISTS practico3_ej2;
SET search_path TO practico3_ej2;

-- Limpieza opcional para re-ejecutar el script en pruebas
...
-- Funcion pedida: sin usar MAX.
-- Usa un recorrido (FOR) para encontrar el mayor valor manualmente.
...
-- Ejecuta la funcion para un banco y retorna la salida.
...
-- Verificacion de actualizacion en tabla banco
...
- Se cumple la restriccion del enunciado: no se usa `MAX`.
- La funcion devuelve el valor por parametro de salida (`OUT p_cotizacion_maxima`).
- Ademas persiste ese valor en `banco.cotizacion_maxima`.
